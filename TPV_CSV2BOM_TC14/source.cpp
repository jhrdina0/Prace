...
std::vector<tag_t> create_item(std::string itemId, std::string itemName, std::string itemType, std::string uom) {
    ...
    if (not ItemExists(itemId)) {
        ...
        // pokud symbol není nalezen, UOM zùstane na defaultní hodnotì (each)
        UOM_find_by_symbol(uom.c_str(), &uom_tag);
        ITEM_set_unit_of_measure(bo, uom_tag);
        if (ReturnCode != ITK_ok) {
            ECHO(("Vytvoøení položky selhalo ERROR CODE: %d\n", ReturnCode));
        }
...
int TPV_CSV2BOM_TC14(EPM_action_message_t msg)
{
    ...
    // for loop pro každou položku v csv file, pøedpokládá se že všechny následující položky mají ID, Name a Description ve sloupcích [2],[3] a [1]
    for (int i = 3; i < rows; i++) {
        std::vector<tag_t> Tags = create_item(csvData[i][2], csvData[i][3], csvData[i][1], csvData[i][5]);
        tag_t Item = Tags[0];
        tag_t Rev = Tags[1];

        // pøedpokládá se že level je uveden ve sloupci [0] a je ve formátu 1.,2..,3...,4....,
        // kod by nefungoval v pøípadì že by byl level vyšší než 9
        ...
        // následnì na položce vytvoøí BOM view (pokuï již neexistuje)
        while (true) {
            ...
            if (level == goalLevel) {
                if (origLevel == 1) {
                    tag_t* Occurrences;
                    AOM_lock(BomViewRev);
                    // pøedpokládá se, že poèet položek je ve sloupci [4]
